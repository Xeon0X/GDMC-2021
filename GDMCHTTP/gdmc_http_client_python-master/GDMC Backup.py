...
def findGround(pos):
    ...
    if runCommand(f"execute unless block %i %i %i air" % tuple(pos)) == "Test failed": # pos is air
        while ground == None:
            if runCommand(f"execute unless block %i %i %i air" % tuple(pos)) == "Test failed": # pos is air
    ...
    elif runCommand(f"execute unless block %i %i %i air" % (pos[0], pos[1]+1, pos[2])) == "Test failed": # pos is the surface
        ground = (pos[0], pos[1], pos[2])

    else: # pos is under the surface
        while ground == None:
            if runCommand(f"execute unless block %i %i %i air" % tuple(pos)) == "Test failed": # pos is air, surface find
...
def TESTsmoothRoads(points):
    ...
    for i in range(len(points)-1):
        distance += sqrt( ((points[i][0]-points[i+1][0])**2)+((points[i][1]-points[i+1][1])**2)+((points[i][2]-points[i+1][2])**2))
    ...
    for i in range(len(line0)):
        ...
        setBlock('blue_concrete', line1[i]) # ICI calculer les lignes par section de routes pour déterminer ce qui doit y avoir en dessou ou au dessu. Pour les pilliers, les faire au niveau des calculs de la largeur.
...
def smoothRoads(points):
    ...
    # "Parcing"
    for i in range(len(line0[0])-1): #line0[0][0],line0[1][0],line0[2][0]... line0[0][1],line0[1][1],line0[2][1]
        for j in range(len(line0)):
            roadA.append(mathLine(line0[j][i], line0[j][i+1], pixelPerfect=False))
            roadB.append(mathLine(line1[j][i], line1[j][i+1], pixelPerfect=False))
        road0.append(roadA)
        road1.append(roadB)
        roadA = []
        roadB = [] 
    ...
    for i in range(len(road0)): # pour toute les parcelles
        # Find what to do depending of the distance between the road and the ground
        groundDistance = listCenter[i+1][1] - findGround(listCenter[i+1])[1]
        eee.append(groundDistance)

        if 0 <= groundDistance <= 7: # remplir jusqu'au sol
            setBlock('red_concrete', (listCenter[i][0], listCenter[i][1]+3, listCenter[i][2]))
        if 7 < groundDistance: # générer pillier
            setBlock('blue_concrete', (listCenter[i][0], listCenter[i][1]+3, listCenter[i][2]))
        if -7 <= groundDistance < 0: # creuser
            pass
        if -7 > groundDistance: # tunnel
            pass
        ...
        for j in range(len(road1[i])): # pour toutes les lignes
            for k in range(len(road1[i][j])): # pour tous les blocks
                ...
                if 0 <= groundDistance <= 7: # remplir jusqu'au sol
                    fillBlock('black_concrete', (road1[i][j][k][0], road1[i][j][k][1], road1[i][j][k][2], road1[i][j][k][0], road1[i][j][k][1]-groundDistance, road1[i][j][k][2]))
    print(eee)
    print(listCenter)
...
def smoothCurveOffset(x,y,z, N=5): # On peut améliorer la précision en trouvant l'arc intérieur et extérieur : relier les points de l'arc et non calculer leur milieu.
    """
    take 3D coordinates
    """
    lineA = []
    ...
    for i in range(len(x)-1):
        ...
        lineA.append(((paralelle[0][0][0], y[i], paralelle[0][0][1]), (paralelle[0][1][0], y[i+1], paralelle[0][1][1])))
        lineB.append(((paralelle[1][0][0], y[i], paralelle[1][0][1]), (paralelle[1][1][0], y[i+1], paralelle[1][1][1])))

    # First points
    line0.append((round(lineA[0][0][0]), round(lineA[0][0][1]), round(lineA[0][0][2])))
    line1.append((round(lineB[0][0][0]), round(lineB[0][0][1]), round(lineB[0][0][2])))

    # Middle of between segments
    for i in range(len(lineA)-1):
        line0.append((round((lineA[i][1][0]+lineA[i+1][0][0])/2), round((lineA[i][1][1]+lineA[i+1][0][1])/2), round((lineA[i][1][2]+lineA[i+1][0][2])/2)))
        line1.append((round((lineB[i][1][0]+lineB[i+1][0][0])/2), round((lineB[i][1][1]+lineB[i+1][0][1])/2), round((lineB[i][1][2]+lineB[i+1][0][2])/2)))

    # Last points
    line0.append((round(lineA[-1][1][0]), round(lineA[-1][1][1]), round(lineA[-1][1][2])))
    line1.append((round(lineB[-1][1][0]), round(lineB[-1][1][1]), round(lineB[-1][1][2])))
...
def worldtrade2(pos1, pos2, blockBase, blockGlass):
    """
    carré pair pour éviter bug
    setBlock not working with use_batching = True sometimes
    problem de temps de génération
    probleme surface diagonale inférieur aux verticales
    """
    # Init
    if pos1 == pos2:
        return(None)
    ...
    # Façades
    # lineBot = []
    # lineBot.append(mathLine((pos1[0], yBot, pos1[2]), (pos1[0], yBot, pos2[2])))
    # lineBot.append(mathLine((pos1[0], yBot, pos1[2]), (pos2[0], yBot, pos1[2])))
    # lineBot.append(mathLine((pos2[0], yBot, pos2[2]), (pos1[0], yBot, pos2[2])))
    # lineBot.append(mathLine((pos2[0], yBot, pos2[2]), (pos2[0], yBot, pos1[2])))
...
def worldtrade(pos1, pos2, blockBase, blockGlass):
    """
    setBlock not working with use_batching = True
    problem de temps de génération
    probleme surface diagonale inférieur aux verticales
    """
    # Init
    ...
    lineTop = []
    lineTop.append(mathLine((center[0], yTop, pos1[2]), (pos1[0], yTop, center[1])))
    lineTop.append(mathLine((pos1[0], yTop, center[1]), (center[0], yTop, pos2[2])))
    lineTop.append(mathLine((center[0], yTop, pos2[2]), (pos2[0], yTop, center[1])))
    lineTop.append(mathLine((pos2[0], yTop, center[1]), (center[0], yTop, pos1[2])))
...
def skyscrapers(pos1, pos2, blockBase, blockGlass, blockFloor):
    ...
    for zPattern in range(pos1[2]+stepZ, pos2[2], stepZ*2):
        fillBlock(blockBase, (pos1[0]+stepX, yBot, zPattern, pos1[0], yTop, zPattern))
    for zPattern in range(pos1[2]+stepZ*2, pos2[2], stepZ*2):
        fillBlock(blockGlass, (pos1[0]+stepX, yBot+1, zPattern, pos1[0]+stepX, yTop, zPattern))
    for xPattern in range(pos1[0]+stepX, pos2[0], stepX*2):
        fillBlock(blockBase, (xPattern, yBot, pos1[2]+stepZ, xPattern, yTop, pos1[2]))
    for xPattern in range(pos1[0]+stepX*2, pos2[0], stepX*2):
        fillBlock(blockGlass, (xPattern, yBot+1, pos1[2]+stepZ, xPattern, yTop, pos1[2]+stepZ))

    for zPattern in range(pos1[2]+stepZ, pos2[2], stepZ*2):
        fillBlock(blockBase, (pos2[0]-stepX, yBot, zPattern, pos2[0], yTop, zPattern))
    for zPattern in range(pos1[2]+stepZ*2, pos2[2], stepZ*2):
        fillBlock(blockGlass, (pos2[0]-stepX, yBot+1, zPattern, pos2[0]-stepX, yTop, zPattern))
    for xPattern in range(pos1[0]+stepX, pos2[0], stepX*2):
        fillBlock(blockBase, (xPattern, yBot, pos2[2]-stepZ, xPattern, yTop, pos2[2]))
    for xPattern in range(pos1[0]+stepX*2, pos2[0], stepX*2):
        fillBlock(blockGlass, (xPattern, yBot+1, pos2[2]-stepZ, xPattern, yTop, pos2[2]-stepZ))
    ...
    while stage+7 < yTop:
        stage += 7
        fillBlock(blockFloor, (pos1[0]+stepX, stage, pos1[2]+stepZ, pos2[0]-stepX, stage-1, pos2[2]-stepZ))
    
def hyperions(pos):
    (x, y, z) = pos
    #points = np.array([[x+20, y, z],[x, y, z+15],[x-30, y, z],[x, y, z-20],[x+20, y, z]])
    points = np.array([[x+15, y, z],[x, y, z+30],[x-35, y, z],[x, y, z-15],[x+15, y, z]])
    smoothRoads(points)
...
#skyscrapers((215, 4, -9), (251, 150, -49), "white_concrete", "gray_stained_glass", "gray_concrete")
worldtrade2((420, 4, 101), (420+64, 164, 101+64), "white_concrete", "white_stained_glass")