...
def cleanLine(path):  # WORK BUT NOT ENDS
    """
    Removes the line from the path
...
def newSmoothCurve(points, type, res=7):
    ...
    for i in range(len(line4)):
        for j in range(len(pointsList)):
            if (line4[i][0] == pointsList[j][0]) and (
                line4[i][1] == pointsList[j][2]
            ):
    ...
    for i in range(len(coorPerpendicularMid0) - 1):
        setLine(
            "cobweb",
            (
                coorPerpendicularMid0[i][0],
                curveY[i],
                coorPerpendicularMid0[i][1],
            ),
            (
                coorPerpendicularMid0[i + 1][0],
                curveY[i],
                coorPerpendicularMid0[i + 1][1],
            ),
        )
        setLine(
            "dirt",
            (
                coorPerpendicularMid1[i][0],
                curveY[i],
                coorPerpendicularMid1[i][1],
            ),
            (
                coorPerpendicularMid1[i + 1][0],
                curveY[i],
                coorPerpendicularMid1[i + 1][1],
            ),
        )
...
def offset(N, listPoints):
    ...
    for i in range(len(listPoints) - 1):
        Xa, Ya, Xb, Yb = mathPerpendicular(
            N, (listPoints[i], listPoints[i + 1])
        )
    ...
    for i in range(0, len(coorPerpendicular0)):
        a, b = (
            round((coorPerpendicularA[i][0] + coorPerpendicular1[i][0]) / 2),
            round((coorPerpendicularA[i][1] + coorPerpendicular1[i][1]) / 2),
        )
        c, d = (
            round((coorPerpendicularB[i][0] + coorPerpendicular0[i][0]) / 2),
            round((coorPerpendicularB[i][1] + coorPerpendicular0[i][1]) / 2),
        )
